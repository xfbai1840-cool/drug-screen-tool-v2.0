...
- **支持多板分析**：自动识别板号，按数据块顺序自动匹配。
- **智能清洗**：自动剔除内参异常值和毒性假阳性。
- **化合物库映射**：强力解析 MCE 的刁钻格式，过滤干扰行，精准匹配药物名称！
""")
...
def load_mce_library(file_obj, filename):
    """ 超级健壮的化合物库读取器（带多层容错和垃圾行清洗） """
    try:
        # 1. 暴力全文本读取，避开所有解析引擎的坑
        ...
        # 2. 智能搜寻真实的表头 (寻找同时包含 plate 和 well 的那一行)
        header_idx = -1
        ...
        if header_idx != -1:
            ...
            st.warning("化合物库中未找到 Plate 和 Well 表头，请检查文件。")
            return None
        ...
        for c in df.columns:
            cl = c.lower().strip()
            if cl in ['plate', 'plate number', 'platenum', 'rack']: col_map[c] = 'Plate'
        ...
        # 4. 【核心修复】过滤垃圾行：孔位不是A01格式的（例如带有 μL 希腊字母的注释行）直接删掉
        if 'Well' in df.columns:
            valid_well = df['Well'].fillna("").astype(str).str.strip().str.match(r'^[a-zA-Z]\d{1,2}$')
            df = df[valid_well].copy()
            
        # 5. 【核心修复】板号映射：MCE的板号(HYCPK79555)按顺序映射为绝对物理板号 1, 2, 3...
...
if uploaded_file is not None:
    try:
        ...
        for start_row in range(0, len(df), PLATE_STEP):
            current_step += 1  # 这一块数据属于第几板 (1, 2, 3...)
            ...
            if plate_id == 'nan' or plate_id.strip() == '':
                plate_id = f"Plate_{current_step}"
                
            # 【核心修复】不去看什么 3K 还是 8K，直接强制按读取顺序赋予物理板号 (1, 2, 3...)
            physical_plate = str(current_step)
